...
// Exits with a non-zero code if any problem is found.
...
function main() {
  ...
  // The Archive/Practice menus read quizzes/index.json, so it must list every quiz file.
  try {
    ...
    err("quizzes/index.json not found or invalid (run: npm run build:index)");
    totalErrors++;
  }
